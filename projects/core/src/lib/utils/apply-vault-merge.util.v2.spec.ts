...
import { applyNgVaultValueMergev2 } from './apply-vault-merge.util';
...
describe('Utility: applyVaultValueMerge', () => {
  ...
  it('should safely do nothing when ctx is undefined', () => {
    expect(() => applyNgVaultValueMergev2(undefined as any, [1], [2])).not.toThrow();
  });

  it('should safely do nothing when ctx.value is undefined', () => {
    const badCtx = { value: undefined } as any;
    expect(() => applyNgVaultValueMergev2(badCtx, [1], [2])).not.toThrow();
  });

  it('should safely do nothing when next is undefined', () => {
    const curr = [1, 2, 3];
    expect(applyNgVaultValueMergev2(ctx, curr, undefined)).toBeUndefined();
  });
  ...
  describe('Arrays', () => {
    it('should append arrays together', () => {
      const curr = [1, 2];
      const next = [3, 4];
      expect(applyNgVaultValueMergev2(ctx, curr, next)).toEqual([3, 4]);
    });

    it('should not mutate original arrays', () => {
      const curr = [1, 2];
      const next = [3];
      expect(applyNgVaultValueMergev2(ctx, curr, next)).toEqual([3]);
    ...
    it('should handle empty arrays', () => {
      const curr: any[] = [];
      const next: any[] = [];
      expect(applyNgVaultValueMergev2(ctx, curr, next)).toEqual([]);
    });
  });
  ...
  describe('Objects', () => {
    it('should shallow merge plain objects', () => {
      const curr = { a: 1, b: 2 };
      const next = { b: 3, c: 4 };
      expect(applyNgVaultValueMergev2(ctx, curr, next as any)).toEqual({ a: 1, b: 3, c: 4 });
    });

    it('should replace nested object reference (shallow merge only)', () => {
      const curr = { deep: { one: 1 } };
      const next = { deep: { two: 2 } };
      expect(applyNgVaultValueMergev2(ctx, curr, next as any)).toEqual({ deep: { two: 2 } });
    });

    it('should preserve immutability', () => {
      const curr = { name: 'Ada' };
      const next = { age: 35 } as any;
      expect(applyNgVaultValueMergev2(ctx, curr, next)).toEqual({ name: 'Ada', age: 35 });
  ...
  describe('Mixed-type replacements', () => {
    it('should replace object with array', () => {
      ...
      expect(applyNgVaultValueMergev2(ctx, curr, next)).toEqual([1, 2]);
    });

    it('should replace array with object', () => {
      const curr = [1, 2];
      const next = { a: 1 } as any;
      expect(applyNgVaultValueMergev2(ctx, curr, next)).toEqual({ a: 1 });
    });

    it('should replace primitive with array', () => {
      expect(applyNgVaultValueMergev2(ctx, 42, [7, 8] as any)).toEqual([7, 8]);
    });

    it('should replace primitive with object', () => {
      expect(applyNgVaultValueMergev2(ctx, 'test', { val: 123 } as any)).toEqual({ val: 123 });
    });
  });

  describe('Primitives', () => {
    it('should replace primitive values (number)', () => {
      expect(applyNgVaultValueMergev2(ctx, 1, 2)).toBe(2);
    });

    it('should replace string values', () => {
      expect(applyNgVaultValueMergev2(ctx, 'foo', 'bar')).toBe('bar');
    });

    it('should replace boolean values', () => {
      expect(applyNgVaultValueMergev2(ctx, false, true)).toBeTrue();
    });
  });

  describe('Edge cases', () => {
    it('should handle null current and valid next', () => {
      expect(applyNgVaultValueMergev2(ctx, null, { name: 'Ada' })).toEqual({ name: 'Ada' });
    });

    it('should handle null next', () => {
      expect(applyNgVaultValueMergev2(ctx, { test: 1 }, null)).toBeNull();
    });

    it('should handle NaN gracefully', () => {
      expect(applyNgVaultValueMergev2(ctx, NaN, 42)).toBe(42);
    });
    ...
    it('should handle symbolic and unexpected types gracefully', () => {
      ...
      expect(() => applyNgVaultValueMergev2(ctx, sym as any, 'value')).not.toThrow();
    });
  });
  ...
  describe('Object shallow merge line coverage', () => {
    it('should merge two simple objects and override overlapping keys', () => {
      ...
      const result = applyNgVaultValueMergev2(ctx, curr, next);
    ...
    it('should not throw when merging objects containing functions', () => {
      ...
      expect(() => applyNgVaultValueMergev2(ctx, curr, next)).not.toThrow();
    });
